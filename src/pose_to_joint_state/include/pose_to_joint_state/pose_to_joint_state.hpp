 ...
 * @brief TODO(...)
 */
class PoseToJointState : public moveit_studio::behaviors::SharedResourcesNode<BT::SyncActionNode>
{
public:
  // Port names.
  static constexpr auto kPortIDCurrentPose = "current_pose";
  static constexpr auto kPortIDDesiredPose = "desired_pose";
  static constexpr auto kPortIDTargetJointState = "target_joint_state";
  /**
   * @brief Constructor for the pose_to_joint_state behavior.
   ...
   */
  PoseToJointState(const std::string& name, const BT::NodeConfiguration& config,
                   const std::shared_ptr<moveit_studio::behaviors::BehaviorContext>& shared_resources);

  /**
   * @brief Implementation of the required providedPorts() function for the pose_to_joint_state Behavior.
   ...
   * @return pose_to_joint_state does not expose any ports, so this function returns an empty list.
   */
  ...
  /**
   * @brief Implementation of BT::SyncActionNode::tick() for PoseToJointState.
   * @details This function is where the Behavior performs its work when the behavior tree is being run. Since
   * PoseToJointState is derived from BT::SyncActionNode, it is very important that its tick() function always finishes
